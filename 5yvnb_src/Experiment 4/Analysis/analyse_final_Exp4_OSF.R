...
for(i in c(1:50)){
	...
	if(i < 10){
		file <- sprintf("C:\\Users\\emiel\\Desktop\\Multi-Actor MNS\\Experimenten\\Experimenten\\4H\\Exp4_4H con empty\\Data\\data0%d.txt", i)
	}else{
		file <- sprintf("C:\\Users\\emiel\\Desktop\\Multi-Actor MNS\\Experimenten\\Experimenten\\4H\\Exp4_4H con empty\\Data\\data%d.txt", i)
	}
...
data.descriptive <- cast(ppn + lftd + gesl ~ ., data = data.full, value = "rt", length)
data.descriptive
...
data.rt.exp4.raw <- data.rt
data.corr.exp4.raw <- data.corr
...
barchart(RT ~ aantal, data = data.plot.rt, ylab = "RT", ylim = c(398, 447), scale = list(tick.number = 10),
...
#anova(lme.rt, lme.rt2)

##### fixed effects ######

lme.rt <- lmer(rt ~ aantal_continu + (1 | ppn), data = data.rt, REML = T)
summary(lme.rt)
anova(lme.rt, type = 3)
...
lme.rt.gamm.lin <- gam(rt ~ aantal_continu + s(ordered(data.rt$ppn), bs = "re"), data = data.rt)
summary(lme.rt.gamm.lin)

lme.rt.gamm <- gam(rt ~ s(aantal_continu, k = 4, bs = "tp") + s(ordered(data.rt$ppn), bs = "re"), data = data.rt)
summary(lme.rt.gamm)
...
names(data.predict)[8:9] <- c("predict.lin", "predict.gam")
head(data.predict)
...
#ppi <- 1200
#tiff(filename = "C:\\Users\\emiel\\Desktop\\Fig5_hres.tiff", compression = "zip", width = 7*ppi, height = 7*ppi, 
#     pointsize = 7*(9/5), res = ppi)

barchart(RT ~ aantal, data = data.plot.rt, type = c("l", "p"), col = "grey",
	xlab = list("Number of Observed Movements", cex = 1.1), ylab = list("RT", cex = 1.1),
	ylim = c(408,442), scales = list(tck = c(1,0), y = list(at = round(seq(410, 440, 5)))),
	par.settings = mytheme,
	panel=function(x, y, subscripts, stderr, box.ratio, ...)
	{
...
spat.exp4.raw <- data.analysis.spatial
...
data.rt.exp4$ppn <- data.rt.exp4$ppn + max(data.rt.exp1$ppn)
data.rt.exp4 <- data.rt.exp4[data.rt.exp4$aantal %in% c("One","Two","Three","Four"), ]
data.rt.exp4$aantal <- factor(data.rt.exp4$aantal)
head(data.rt.exp4)
tail(data.rt.exp4)

data.rt.combo14 <- rbind(data.rt.exp1, data.rt.exp4)
length(table(data.rt.combo14$ppn))
...
lme.rt.lin.combo14 <- gam(rt ~ exp + aantal_continu + s(factor(data.rt.combo14$ppn), bs = "re"), data = data.rt.combo14)
summary(lme.rt.lin.combo14)

lme.rt.lin.inter.combo14 <- gam(rt ~ exp*aantal_continu + s(factor(data.rt.combo14$ppn), bs = "re"), data = data.rt.combo14)
summary(lme.rt.lin.inter.combo14)

lme.rt.gamm.combo14 <- gam(rt ~ exp + s(aantal_continu, k = 4, bs = "tp") + s(factor(data.rt.combo14$ppn), bs = "re"), data = data.rt.combo14)
summary(lme.rt.gamm.combo14)

lme.rt.gamm.inter.combo14 <- gam(rt ~ exp + s(aantal_continu, k = 4, bs = "tp") + s(aantal_continu, k = 4, bs = "tp", by = exp) + s(factor(data.rt.combo14$ppn), bs = "re"), data = data.rt.combo14)
summary(lme.rt.gamm.inter.combo14)

###### statistics ######

AIC(lme.rt.lin.combo14, lme.rt.lin.inter.combo14, lme.rt.gamm.combo14, lme.rt.gamm.inter.combo14)
round(AIC(lme.rt.gamm.combo14) - AIC(lme.rt.gamm.inter.combo14),2)
round(AIC(lme.rt.lin.inter.combo14) - AIC(lme.rt.gamm.inter.combo14),2)
round(AIC(lme.rt.lin.combo14) - AIC(lme.rt.gamm.inter.combo14),2)

round(lme.rt.gamm.combo14$gcv.ubre - lme.rt.gamm.inter.combo14$gcv.ubre,2)
round(lme.rt.lin.inter.combo14$gcv.ubre - lme.rt.gamm.inter.combo14$gcv.ubre,2)
round(lme.rt.lin.combo14$gcv.ubre - lme.rt.gamm.inter.combo14$gcv.ubre,2)

anova(lme.rt.lin.combo14, lme.rt.gamm.inter.combo14, test = "Chisq")
anova(lme.rt.lin.inter.combo14, lme.rt.gamm.inter.combo14, test = "Chisq")
anova(lme.rt.gamm.combo14, lme.rt.gamm.inter.combo14, test = "Chisq")

###### plot ######

data.predict.combo14 <- cbind(data.rt.combo14, predict(lme.rt.gamm.inter.combo14))
names(data.predict.combo14)[9] <- c("predict.gam")
head(data.predict.combo14)

data.plot.fit.gam.combo14.a <- cast(ppn + exp + aantal ~ ., data = data.predict.combo14, value = "predict.gam", mean)
data.plot.fit.combo14.gam <- cast(exp + aantal ~ ., data = data.plot.fit.gam.combo14.a, mean)
names(data.plot.fit.combo14.gam)[3] <- "predict.gam"

xyplot(predict.gam ~ aantal | exp, type = c("spline", "p"), data = data.plot.fit.combo14.gam, lwd = 2, par.settings = simpleTheme(col = "black", lty = 1, pch = 19))

	###### RT COMPARISON ######

data.rt.comp1 <- cast(ppn ~ ., data = data.rt.exp1, value = "rt", mean)
names(data.rt.comp1)[2] <- "RT"
data.rt.comp4 <- cast(ppn ~ ., data = data.rt.exp4, value = "rt", mean)
names(data.rt.comp4)[2] <- "RT"

t.test(data.rt.comp1$RT, data.rt.comp4$RT, var.equal = F)
cohensD(data.rt.comp1$RT, data.rt.comp4$RT, method = "unequal")
...
barchart(ErrorRate*100 ~ aantal, data = data.plot.err, ylab = "Error Rate", ylim = c(-0.4, 6.4), scale = list(tick.number = 20))
