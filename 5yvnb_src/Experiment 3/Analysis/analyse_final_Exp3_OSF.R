...
for(i in c(1:48)){
	...
	if(i < 10){
		file <- sprintf("C:\\Users\\emiel\\Desktop\\Multi-Actor MNS\\Experimenten\\Experimenten\\4H\\Exp3_4H con\\Data\\data0%d.txt", i)
	}else{
		file <- sprintf("C:\\Users\\emiel\\Desktop\\Multi-Actor MNS\\Experimenten\\Experimenten\\4H\\Exp3_4H con\\Data\\data%d.txt", i)
	}
...
data.rt.exp3.raw <- data.rt
data.corr.exp3.raw <- data.corr
...
barchart(RT ~ aantal, data = data.plot.rt, ylab = "RT", ylim = c(348, 402), scale = list(tick.number = 10),
...
#ppi <- 1200
#tiff(filename = "C:\\Users\\emiel\\Desktop\\Fig4_hres.tiff", compression = "zip", width = 7*ppi, height = 7*ppi, 
...
barchart(RT ~ aantal, data = data.plot.rt, col = "grey",
	...
	ylim = c(363,392), scales = list(tck = c(1,0), y = list(at = round(seq(365, 390, 5)))),
	par.settings = mytheme,
	panel=function(x, y, subscripts, stderr, box.ratio, ...)
	{
...
spat.exp3.raw <- data.analysis.spatial
...
data.rt.exp3$ppn <- data.rt.exp3$ppn + max(data.rt.exp1$ppn)
head(data.rt.exp3)
tail(data.rt.exp3)

data.rt.combo13 <- rbind(data.rt.exp1, data.rt.exp3)
head(data.rt.combo13)
length(table(data.rt.combo13$ppn))
...
lme.rt.lin.combo13 <- gam(rt ~ exp + aantal_continu + s(factor(data.rt.combo13$ppn), bs = "re"), data = data.rt.combo13)
summary(lme.rt.lin.combo13)

lme.rt.lin.inter.combo13 <- gam(rt ~ exp*aantal_continu + s(factor(data.rt.combo13$ppn), bs = "re"), data = data.rt.combo13)
summary(lme.rt.lin.inter.combo13)

lme.rt.gamm.combo13 <- gam(rt ~ exp + s(aantal_continu, k = 4, bs = "tp") + s(factor(data.rt.combo13$ppn), bs = "re"), data = data.rt.combo13)
summary(lme.rt.gamm.combo13)

lme.rt.gamm.inter.combo13 <- gam(rt ~ exp + s(aantal_continu, k = 4, bs = "tp") + s(aantal_continu, k = 4, bs = "tp", by = exp) + s(factor(data.rt.combo13$ppn), bs = "re"), data = data.rt.combo13)
summary(lme.rt.gamm.inter.combo13)

###### statistics ######

AIC(lme.rt.lin.combo13, lme.rt.lin.inter.combo13, lme.rt.gamm.combo13, lme.rt.gamm.inter.combo13)
round(AIC(lme.rt.gamm.combo13) - AIC(lme.rt.gamm.inter.combo13),2)
round(AIC(lme.rt.lin.inter.combo13) - AIC(lme.rt.gamm.inter.combo13),2)
round(AIC(lme.rt.lin.combo13) - AIC(lme.rt.gamm.inter.combo13),2)

round(lme.rt.gamm.combo13$gcv.ubre - lme.rt.gamm.inter.combo13$gcv.ubre,2)
round(lme.rt.lin.inter.combo13$gcv.ubre - lme.rt.gamm.inter.combo13$gcv.ubre,2)
round(lme.rt.lin.combo13$gcv.ubre - lme.rt.gamm.inter.combo13$gcv.ubre,2)

anova(lme.rt.lin.combo13, lme.rt.gamm.inter.combo13, test = "Chisq")
anova(lme.rt.lin.inter.combo13, lme.rt.gamm.inter.combo13, test = "Chisq")
anova(lme.rt.gamm.combo13, lme.rt.gamm.inter.combo13, test = "Chisq")

###### plot ######

data.predict.combo13 <- cbind(data.rt.combo13, predict(lme.rt.gamm.inter.combo13))
names(data.predict.combo13)[9] <- c("predict.gam")
head(data.predict.combo13)

data.plot.fit.gam.combo13.a <- cast(ppn + exp + aantal ~ ., data = data.predict.combo13, value = "predict.gam", mean)
data.plot.fit.combo13.gam <- cast(exp + aantal ~ ., data = data.plot.fit.gam.combo13.a, mean)
names(data.plot.fit.combo13.gam)[3] <- "predict.gam"

xyplot(predict.gam ~ aantal | exp, type = c("spline", "p"), data = data.plot.fit.combo13.gam, lwd = 2, par.settings = simpleTheme(col = "black", lty = 1, pch = 19))

	###### RT COMPARISON ######

data.rt.comp1 <- cast(ppn ~ ., data = data.rt.exp1, value = "rt", mean)
names(data.rt.comp1)[2] <- "RT"
data.rt.comp3 <- cast(ppn ~ ., data = data.rt.exp3, value = "rt", mean)
names(data.rt.comp3)[2] <- "RT"

t.test(data.rt.comp1$RT, data.rt.comp3$RT, var.equal = F)
cohensD(data.rt.comp1$RT, data.rt.comp3$RT, method = "unequal")
