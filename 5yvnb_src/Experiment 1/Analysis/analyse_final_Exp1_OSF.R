...
for(i in c(1:42)){
	...
	if(i < 10){
		file <- sprintf("C:\\Users\\emiel\\Desktop\\Multi-Actor MNS\\Experimenten\\Experimenten\\4H\\Exp1_4H\\Data\\data0%d.txt", i)
	}else{
		file <- sprintf("C:\\Users\\emiel\\Desktop\\Multi-Actor MNS\\Experimenten\\Experimenten\\4H\\Exp1_4H\\Data\\data%d.txt", i)
	}
...
data.descriptive <- data.descriptive[data.descriptive$ppn != 30, ] #excluded from sample (see below)
...
data.outlier <- data.full[data.full$ppn != 30, ]
...
data.rt <- data.rt[data.rt$ppn != 30, ]
data.corr <- data.corr[data.corr$ppn != 30,]

#data.rt <- data.rt[data.rt$ppn != 41, ] #what happens if last two participants are removed from sample? (--> N = 40)
#data.rt <- data.rt[data.rt$ppn != 42, ] #what happens if last two participants are removed from sample? (--> N = 40)

data.rt.exp1.raw <- data.rt
data.corr.exp1.raw <- data.corr
...
SE_Within <- function(data, colnames, ref = F){
	...
	output <- data.frame(row.names = colnames, sd.between, se.between, ci95.between, se.within, ci95.within,
...
barchart(RT ~ aantal, groups = con, data = data.plot.rt,
	ylim = c(397,448), scales = list(tck = c(1,0), y = list(at = round(seq(400, 445, 5)))),
...
barchart(Congruency ~ aantal, data = data.plot.rt.diff, ylab = "Congruency Effect", ylim = c(-1.5, 41.5), scale = list(tick.number = 10),
	   par.settings = mytheme,
...
#lme.rt <- lmer(rt ~ con*aantal_continu + (1 + con + aantal_continu + con:aantal_continu || ppn), data = data.rt, REML = F)
	#does not converge with our without correlations
...
anova(lme.rt.gamm.lin, lme.rt.gamm, test = "Chisq")
...
#ppi <- 1200
#tiff(filename = "C:\\Users\\emiel\\Desktop\\Fig2_hres.tiff", compression = "zip", width = 7*ppi, height = 7*ppi, 
#     pointsize = 7*(9/5), res = ppi)

barchart(RT ~ aantal, groups = con, data = data.plot.rt,
	ylim = c(403,442), scales = list(tck = c(1,0), y = list(at = round(seq(400, 440, 5)))), box.ratio = 3,
	xlab = list("Number of Observed Movements", cex = 1.1), ylab = list("RT", cex = 1.1),
...
spat.exp1.raw <- data.analysis.spatial
